# -*- coding: cp936 -*-
import os,time,socket,sys,re,datetime


def check_process_status(restart_time,sleeptime,local_dir,exe,port = '',is_game = '0'):
    
    local_time=datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')
    local_time_min=datetime.datetime.today().strftime('%H%M%S')
    if float(local_time_min)>= float(restart_time) and float(local_time_min)<= float(restart_time)+sleeptime:
