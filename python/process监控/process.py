...
while 1:
    ...
    #if local_time_min == restart_time:
    #    start_process(local_dir,exe)
    #    log_str = local_time+" "+exe+" NOW RESTARTING..."
    #else:
    
    if not cf.get("main","restarttime"):
        restart_time = '010000'
    else:
        restart_time = cf.get("main","restarttime")
    print float(restart_time)+sleeptime
    print "restarttime is every day's "+restart_time
    ...
    for i in range(process_nu):
        ...
        if "port" in cf.options(sec):
            ...
            if "is_game" in cf.options(sec) and cf.get(sec,"is_game") == '1':
                ...
                game_check = '0'
            #print 
            log_str = pscheck.check_process_status(restart_time,sleeptime,local_dir,exe,port,game_check)
            #print "port is " + port
        else:
            log_str = pscheck.check_process_status(restart_time,sleeptime,local_dir,exe)
        f.write(log_str+"\r\n")
        print log_str
