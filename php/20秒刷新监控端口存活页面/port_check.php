...
function PortCheck($targetIP,$targetPort,$targetItem,$Protocol="tcp",$echoChar=NULL,$timeout=5,$number=1){
    $i     = 0;
    $j    = 0;
    while ($i++<$number) {
    ...
    $link=mysql_connect("10.48.179.112","dc","dc") or die("connect error");
    mysql_select_db("gp_platform") or die("select db error");
    $sql="select item_host,item_port,item_name from monitor_moniter_port_list where item_on_check = '1' ;";
    ...
    while($row=mysql_fetch_assoc($result)){
        echo "<tr>";
        PortCheck($row['item_host'],$row['item_port'],$row['item_name']);
        echo "<tr>";
    }
    
//PortCheck('ip','888', 'bad!');
